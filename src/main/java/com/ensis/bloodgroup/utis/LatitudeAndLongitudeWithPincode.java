...
public class LatitudeAndLongitudeWithPincode {
	...
	public static String[] getLatLongPositions(String address) throws Exception {
		try {
			...
			String api = "http://maps.googleapis.com/maps/api/geocode/xml?address="
					...
					+ "&sensor=true;key=AIzaSyAYWLQsRixGvdjyBpeL1IzzYPxdtFWQuOA";
	 ...
	 * System.out.println("Please enter a location:"); String
	 * postcode=reader.readLine(); String latLongs[] =
	 * getLatLongPositions(postcode);
	 * System.out.println("Latitude: "+latLongs[0]
	 * +" and Longitude: "+latLongs[1]); }
	 * 
	 * public static String[] getLatLongPositions(String address) throws
	 * Exception { int responseCode = 0; String api =
	 * "https://maps.googleapis.com/maps/api/geocode/xml?address=" +
	 * URLEncoder.encode(address, "UTF-8") + "&sensor=true"; URL url = new
	 * URL(api); HttpURLConnection httpConnection =
	 * (HttpURLConnection)url.openConnection(); httpConnection.connect();
	 * responseCode = httpConnection.getResponseCode(); if(responseCode == 200)
	 * { DocumentBuilder builder =
	 * DocumentBuilderFactory.newInstance().newDocumentBuilder();; Document
	 * document = builder.parse(httpConnection.getInputStream()); XPathFactory
	 ...
	 * (String)expr.evaluate(document, XPathConstants.STRING); return new
	 * String[] {latitude, longitude}; } else { throw new
	 * Exception("Error from the API - response status: "+status); } } return
	 * null; }
	 */
