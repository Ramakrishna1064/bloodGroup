...
public class ListOfRequestsService extends MessageResources {
	...
	public List<GetBloodRequestsModel> getListOfRequests(int donorId)
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if(requestObj.getRequeststatus()==2)
			{
				if(getToDateAfterDays(request.getDatebloodrequired()).compareTo(new Date())>0)
				{
					bloodRequestObj.add(getBloodRequestsModel);
				}
			}
			else if(requestObj.getRequeststatus()==5)
			{
				bloodRequestObj.add(getBloodRequestsModel);
	...
	public Date getToDateAfterDays(Date nowdate) {
